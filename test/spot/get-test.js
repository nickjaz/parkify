...
describe('ENHC GET: ${url}/api/lot/${this.lot._id}/spot', function() {
  beforeEach( done => {
    generateUser()
    .then( tempHost => {
  ...
  describe('Valid Request', () => {
    it('should return a 200 status code', done => {
      request.get(`${url}/api/lot/${this.lot._id}/spot/${this.spot._id}`)
      .set({
  ...
  describe('nonexistent id', () => {
    it('should return a 404 status code', done => {
      request.get(`${url}/api/lot/${this.lot._id}/spot/1234567890`)
      .set({
        Authorization: `Bearer ${this.hostToken}`
      })
      .end((error, response) => {
